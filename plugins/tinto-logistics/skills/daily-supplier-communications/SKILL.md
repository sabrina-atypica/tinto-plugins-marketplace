...
- If `Confidence` is **"Draft estimate — needs confirming"**, check the row's `Notes` field for a `[PAIRING CONFIRMED BY TAMARA ...]` or `[PAIRING CORRECTED BY TAMARA ...]` marker (written by the `confirm-supplier-pairings` skill).
  - Marker present → the pairing has already been reviewed and confirmed (or corrected — the `Supplier` link will already point at the right record). Proceed normally.
  - No marker → **do not draft.** This pairing was built by matching the tour's region against the supplier's region, not from a confirmed itinerary, and hasn't been checked by a human yet. Leave `Booking Request Status` as it is (don't move it to "Due — Draft Needed" — nothing was drafted), and instead add it to a distinct section of the end-of-run ping: "awaiting pairing confirmation." Name the tour, the supplier, and say plainly that running `confirm-supplier-pairings` for that tour's destination would resolve it.

This gate applies every run, not just the first one — new tours get added over time, and each one starts with the same region-matched, unconfirmed pairings until someone runs the audit for its destination.
...
- **Any rows that were due but skipped by the pairing gate above** — name each tour/supplier pair, and say plainly that `confirm-supplier-pairings` needs to run for that destination before this one can draft. Don't bury this among the drafted ones; it's a different kind of item (blocked on her review, not waiting on her to send).
