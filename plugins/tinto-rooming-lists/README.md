...
**Third pass, 2026-09-16:** live-testing the rebuilt skill against a real, currently-booked tour ("Tinto Presents the Joys of Southern Italy — Puglia," Sep 26–Oct 3 2027) surfaced a real design gap — that tour is a 3-city routing (Bari, Ostuni, Lecce), each with its own hotel, which the single-hotel design had no explicit handling for. The skill now builds **one workbook per hotel** for a multi-stop tour, using the `Tour Accommodation` table (linked from the Tour via `Accommodation Stops`) as the authoritative source of which hotels are on record — each stop row links directly to its own `Suppliers` record, which is now the primary path to that stop's language (a real link, not a text match), with the old Property-Name fuzzy match demoted to a fallback for single-stop tours with no `Tour Accommodation` row. Every stop's workbook is built from the same full reservation-based room/guest roster, on the assumption that the group keeps its room assignments for the whole tour — flagged in the skill as an assumption to escalate against, not a confirmed fact, since Tinto hasn't yet confirmed whether that ever isn't true.

**Fourth pass, 2026-09-16, same day:** a full end-to-end build against a real single-hotel tour ("Tinto Presents the Joys of Portugal — the Alentejo," 6 reservations, 11 guests) surfaced one more gap — when a shared room's guests are *both* still missing passport data, their two passport-column rows were identical bare fallback markers ("AT HOTEL"/"AT HOTEL"), with nothing on the row itself saying which guest was which. Fixed: the marker now always includes that guest's own name first (from `Participants.First Name`/`Last Name`, reliably on file long before passport data is) — "Jennifer Howse (AT HOTEL)" rather than a bare "AT HOTEL" — reverting to the plain passport name with no marker once real data comes in.

**Fifth pass, 2026-09-16, same day:** a repeat request for "the Puglia tour" was answered by silently reusing the specific departure identified earlier in the conversation, without re-confirming — and there turn out to be **three separate Puglia departures on record** (Aug 29–Sep 5 2027, Sep 12–19 2027, Sep 26–Oct 3 2027), each with its own distinct bookings. Sabrina caught this directly: the skill should never assume, and should always ask when in doubt, even on what looks like a repeat of an earlier request in the same conversation. New rule added, ahead of everything else in the skill: confirm which specific Tour record is meant whenever a tour name matches more than one departure, surfacing each match's date range, and treat every request as needing its own confirmation rather than inheriting one from earlier in the conversation.

**Sixth pass, 2026-09-16, same day:** testing against the confirmed Sep 12–19 2027 Puglia departure surfaced real Notes text sitting past the `Room N.` tag that the skill was silently discarding — "Will room with Pam (Room 8) if Dick Kennedy can't go" (a live rooming contingency) and, from an earlier test, "Friends sharing 2 beds" (real bed-configuration information no structured field captures). Sabrina asked for this to become a footnote on the sheet plus a direct ping to Tamara to review those specific rooms before sending. Now built in: any leftover Notes text past the room-number tag becomes a footnote block below the TOTAL row, keyed by room number, for every room that has one — not just ones that look obviously relevant, since only Tamara knows which still apply — and the reply itself calls out which rooms have footnotes worth a review before the list goes out. This is still a design-level rebuild verified against real schema and real tours' data, not yet a full end-to-end run confirmed with Tamara or against a live hotel send.
