...
**Nearly everything asked below is booking-page content, not private bookkeeping.** `Overview`, `Price per Person`, `Max Participants`, `Pickup / Drop-off Location`, `What's Included` / `What's Not Included`, `Tour Highlights`, and `Hero Facts` are all read directly by the guest-facing booking page once this tour goes live. Say that plainly while asking, something like "this is exactly what the booking page will show guests for X", rather than letting the confirmation read like routine internal data entry; a wrong or guessed answer here is a guest-facing error the moment the tour publishes, not just an incomplete Airtable row.
...
By the time this skill runs, the hotel hold behind this tour should already have been converted from Available to Sold and bridged into a `Room Blocks` row by `tinto-batch-book-hotel-rooms` (its Half 2), since the winery saying yes is what triggers both that conversion and this tour's creation, in parallel. This skill never touches `Room Blocks.Capacity` itself, that stays that skill's territory entirely, but it does close the one piece neither skill owns yet: turning that capacity into real, `Room Block`-backed priced `Packages` rows. A price may already have been shown to the winery earlier, during the sales pitch itself, since `tinto-client-itinerary-pdf` asks Peter directly for a precedent-anchored figure when no `Room Block` exists yet to read `Packages` from. This step is where that figure, if one was already confirmed for this pitch, gets reconfirmed and made official as real `Packages`, not where pricing first gets invented.
...
2. **If a `Room Block` is linked, first check whether this `Tours` record already carries a confirmed figure from an earlier pitch-stage PDF.** `tinto-client-itinerary-pdf` may already have asked Peter for `Price per Person` and a single-supplement number before this tour was ever sold, since it needs a price to show a winery mid-pitch and had no `Room Block` yet to read `Packages` from; look for that `Price per Person` value and any `[SINGLE SUPPLEMENT CONFIRMED ...]` marker in this Tour's `Notes`. **If found, that's the anchor**: show Peter those numbers and ask him to reconfirm they're still correct, rather than a fresh precedent lookup, since he already answered this exact question once for this same departure. **If nothing was recorded** (no pitch-stage PDF ran ahead of the sale), look up the Double and Solo `Packages` prices from the most recent existing tour at the same destination instead, the same template-reuse convention used everywhere else in this skill, rather than deriving anything from the hotel's own rate quote. Either way, the price a guest actually pays is Peter's own figure, not a pass-through of what the hotel charges, and he updates it roughly once a year, so a prior tour's price (or his own earlier pitch-stage answer) is the safest anchor, never assumed current on its own.
3. **Ask Peter directly, naming the anchor and its actual figures**: either "here's what you confirmed for this tour's own pitch itinerary, Double $X / Solo $Y, still correct?" or, when there was no pitch-stage figure, "here's what [prior tour] charged for Double and Solo at [destination], is this still correct, or has the price changed?" Only once he confirms or corrects both figures, and any other room-type or Bed Configuration variant the template's Room Block covers, does anything get written.
